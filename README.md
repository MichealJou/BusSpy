...
- 更新提示：启动时检测 GitHub Releases 最新版本，发现新版本后提示下载
...
Windows 扫不到串口时，通常先检查：

- 是否已经插入 USB 转串口设备。
- 设备管理器里是否出现 `COM3`、`COM4` 等端口。
- CH340、CP210x、FTDI 等芯片驱动是否已安装。
- 串口是否被其他串口助手、IDE、烧录工具占用。
- 点击 BusSpy 左侧 `刷新端口` 重新扫描。
...
BusSpy 启动时会检查 GitHub Releases 最新版本。

- 如果发现新版本，页面右上角会出现更新提示按钮，显示当前版本和最新版本。
- 点击 `打开下载页` 会用系统默认浏览器跳转到 GitHub Releases。
- 可以选择 `稍后提醒`，下次启动还会继续检查。
- 可以选择 `忽略此版本`，当前版本不再提醒。
- 也可以在 `关于` 菜单中点击 `检查更新` 手动检测。
- 如果当前已经是最新版本，会提示无需更新。

当前更新功能只负责提示和跳转下载页，不会后台静默安装，也不会修改系统文件。

### 8. 关于与版本协议

顶部工具栏的 `关于` 是一个菜单，可以选择：

- `关于 BusSpy`：查看当前版本、作者和支持平台
- `检查更新`：手动检测 GitHub Releases 最新版本
- `版本协议`：查看版本号规范
- `更新策略`：查看更新提示和用户选择规则
- `打开发布页`：进入 GitHub Releases 下载页
...
- `MINOR`：兼容旧版本的新功能
- `PATCH`：问题修复和小优化
- 预发布版本：`alpha` / `beta` / `rc`，例如 `v0.2.0-beta.1`
